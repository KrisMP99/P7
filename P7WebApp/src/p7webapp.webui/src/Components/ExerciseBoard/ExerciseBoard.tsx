    ...
    return (
        <div className='exercise-wrapper'>
            <form onSubmit={(e) => {
                ...
                setIsFetching(true);
                createOrUpdateExercise(exercise, props.exerciseGroupId, props.isNewExercise, (exId) => {
                    if (exId && props.isNewExercise) {
                        navigator('/course/' + props.courseId + '/exercise-group/' + props.exerciseGroupId + '/exercise/' + exId + '/1');
                    }
                    setIsFetching(false);
                });
            }}>
                <div className='board-actions-container'>
                    ...
                    />
                    <Button variant='success' type='submit' disabled={isFetching}>
                        {isFetching ?
                            (<><Spinner
                                as="span"
                                animation="border"
                    ...
                    <Button variant='danger' disabled={isFetching} onClick={() => navigator(-1)}>
                        Cancel
                    </Button>
                </div>
            </form>
...
async function createOrUpdateExercise(exercise: Exercise, exerciseGroupId: number, isNewExercise: boolean, callback: (newExId: number | null) => void) {
        ...
        switch (modules[i].type) {
            case ModuleType.EXERCISE_DESCRIPTION:
                convertedModules.push({
                    id: isNewExercise ? 0 : modules[i].id,
